...
function buildUpdateQuery(speciesData, oldTaxonId, dbColumns) {
  ...
  // Get all mapped columns that exist in database, except matching key and timestamp
  const columns = Object.keys(speciesData).filter(col =>
    col !== 'taxon_full' &&
    col !== 'taxon_id' && // Don't update taxon_id
    col !== 'last_updated_date' && // We set this separately with NOW()
    dbColumns.has(col)
  );
...
function buildInsertQuery(speciesData, dbColumns) {
  // Only include columns that exist in database, excluding timestamp
  const columns = Object.keys(speciesData).filter(col =>
    dbColumns.has(col) && col !== 'last_updated_date'
  );
  const placeholders = columns.map((_, idx) => `$${idx + 1}`).join(', ');
  const values = columns.map(col => speciesData[col]);

  const query = `
    INSERT INTO species (${columns.join(', ')}, last_updated_date)
