#!/usr/bin/env python3
"""Compute normalization stats for the AE-only v50 experiment."""
...
def main() -> None:
    ...
    cont_path = out_dir / "normalize_stats_v50_ae_only.npz"
    temp_path = out_dir / "normalize_temporal_v50_ae_only.npz"
    manifest_path = out_dir / "stats_contract_v50_ae_only.json"

    client = bigquery.Client(project=PROJECT)
    cont_mean, cont_std, n_rows = compute_stats_from_bq(client, SOURCE_TABLE, AE_EMB_COLS)
    temp_mean, temp_std, _ = compute_stats_from_bq(client, SOURCE_TABLE, AE_TEMPORAL_COLS)

    np.savez(cont_path, mean=cont_mean, std=cont_std, columns=np.array(AE_EMB_COLS))
    np.savez(temp_path, mean=temp_mean, std=temp_std, columns=np.array(AE_TEMPORAL_COLS))

    manifest = {
        "contract_name": "sinr_v50_ae_only_normalization",
        "version": "v50_ae_only",
        "created_at_utc": datetime.utcnow().isoformat() + "Z",
        "source_table": SOURCE_TABLE,
        "total_rows": int(n_rows),
        "continuous_cols": len(AE_EMB_COLS),
        "ae_emb_cols": len(AE_EMB_COLS),
        "env_continuous_cols": 0,
