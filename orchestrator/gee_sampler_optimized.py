"""
GEE AlphaEarth Sampler - OPTIMIZED FOR QUOTA MANAGEMENT
Dynamically manages GEE concurrent tasks and batch sizes to maximize throughput
while staying within quota limits.

Key optimizations:
- Dynamic concurrent task management (2-8 tasks based on quota)
- Adaptive batch sizing (500-5000 points per task)
- Automatic retry with exponential backoff
- Real-time quota monitoring

Following: treekipedia_alpha_earth_planetary_species_predictor_builders_guide.md Section 3.4
"""
...
MAX_CONCURRENT_TASKS = 8  # Start conservative
MIN_CONCURRENT_TASKS = 2  # Never go below this
INITIAL_BATCH_SIZE = 2000  # Points per task
MIN_BATCH_SIZE = 500
MAX_BATCH_SIZE = 5000
...
class QuotaManager:
    ...
    """
    def __init__(self):
        self.concurrent_tasks = 4  # Start in middle
    ...
    def adjust_on_success(self):
        ...
        # Gradually increase concurrency after 3 successes
        ...
        # Increase batch size if consistently succeeding
    ...
    def adjust_on_failure(self, error_msg: str):
        ...
        if is_quota_error:
            ...
            return 60  # Return backoff time in seconds
        else:
...
def export_batch_to_bigquery(
    ...
    Uses dynamic batch sizing based on quota manager.

    Args:
        batch_id: Unique batch identifier
        points: List of {taxon_id, latitude, longitude, year, embedding_year}
        max_concurrent: Override concurrent task limit (for testing)

    Returns:
        List of GEE task IDs
    """
    # Use quota manager's batch size
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("GEE SAMPLER TEST - Single Point")
    print("="*70)
    ...
    task_ids = export_batch_to_bigquery('test_single', test_points, max_concurrent=1)
